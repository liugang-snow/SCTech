...
(function ($) {
    $.extend({
    	...
    	modal: {
            ...
            //2020-03-02 LG*
            // 弹出层指定宽度，有保存、提交、关闭三个按钮
            openSave: function (title, url, width, height, callbacksave, callbacksubmit) {
            	//如果是移动端，就使用自适应大小弹窗
            	if (navigator.userAgent.match(/(iPhone|iPod|Android|ios)/i)) {
            	    width = 'auto';
            	    height = 'auto';
                ...
                if ($.common.isEmpty(callbacksave)) {
                	callbacksave = function(index, layero) {
                        var iframeWin = layero.find('iframe')[0];
                        iframeWin.contentWindow.saveHandler(index, layero);
                    }
                }
                if ($.common.isEmpty(callbacksubmit)) {
                	callbacksubmit = function(index, layero) {
                        var iframeWin = layero.find('iframe')[0];
                        iframeWin.contentWindow.submitHandler(index, layero);
                    }
                }
        ...
        operate: {
            ...
            addSave: function(id) {
            	table.set();
            	$.modal.openSave("添加" + table.options.modalName, $.operate.addUrl(id));
            },
            //2020-03-02 LG*
            //修改信息，有保存、提交、关闭三个按钮
            editSave: function(id) {
            	...
            	if($.common.isEmpty(id) && table.options.type == table_type.bootstrapTreeTable) {
                    ...
                    var url = table.options.updateUrl.replace("{id}", row[table.options.uniqueId]);
                    $.modal.openSave("修改" + table.options.modalName, url);
            	} else {
            	    $.modal.openSave("修改" + table.options.modalName, $.operate.editUrl(id));
            	}
            }
        },
        // 校验封装处理
